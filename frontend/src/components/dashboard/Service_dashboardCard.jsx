...
const DashboardCard = ({ data, onClick}) => {
  ...
  const statusCircleStyle = {
    width: "12px",
    height: "12px",
    backgroundColor: getStatusColor(data.serviceStatus),
    borderRadius: "50%",
  ...
  return (
    <div style={cardStyle} onClick={onClick}>
      <img src={data.serviceImage || testImage} className="object-center rounded-2xl w-[140px] h-[140px]"></img>
      ...
      <div style={statusBoxContainerStyle}>
        <div style={statusBox}>
          ...
          <span style={statusTextStyle}>{data.serviceStatus}</span>
        </div>
      </div>
    </div>
  );
